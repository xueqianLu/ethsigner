...
func main() {
	...
	fmt.Println("2. Creating a new account...")
	newAccount, err := c.CreateAccount()
	if err != nil {
	...
	// Use the newly created account for the next steps
	...
	legacyTxReq := handler.SignTxRequest{
		From:     signerAddress,
	...
	eip1559TxReq := handler.SignTxRequest{
		From:      signerAddress,
	...
	messageReq := handler.SignMessageRequest{
		From:    signerAddress,
		Message: "Hello, Vault!",
	}
